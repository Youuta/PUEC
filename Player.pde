public class Player {
  ...
  public void shootWeaponN() {
    ...
    for (int i = 1; i<weaponStatus[this.weapon][1]+1&& this.y -i > -1; i++) {
      if (battleGround[this.x][this.y -i].structure == 0) {
        battleGround[this.x][this.y -i].bullet = this.weapon;
  ...
  public void shootWeaponS() {
    battleGround[this.x][this.y].battleSound = 1;
    for (int i = 1; i<weaponStatus[this.weapon][1]+1 && this.y+i < battleGround.length; i++) {
      if (battleGround[this.x][this.y +i].structure == 0) {
        battleGround[this.x][this.y +i].bullet = this.weapon;
      } else if (battleGround[this.x ][this.y +i].structure == 1) {
        return ;
  ...
  public void shootWeaponW() {
    battleGround[this.x][this.y].battleSound = 1;
    for (int i = 1; i<weaponStatus[this.weapon][1]+1 && this.x -i > -1; i++) {
      if (battleGround[this.x -i][this.y ].structure == 0) {
        battleGround[this.x-i][this.y ].bullet = this.weapon;
  ...
  public void shootWeaponE() {
    battleGround[this.x][this.y].battleSound = 1;
    for (int i = 1; i<weaponStatus[this.weapon][1]+1 && this.x +i < battleGround.length; i++) {
      if (battleGround[this.x +i][this.y ].structure == 0) {
        battleGround[this.x + i][this.y ].bullet = this.weapon;
      } else if (battleGround[this.x +i][this.y ].structure == 1) {
        return ;
  ...
  public int checkWeapon() {
    return battleGround[this.x][this.y].weapon;
  }
  public int checkItem() {
    return battleGround[this.x][this.y].item;
  }
  ...
  boolean seachingOtherPlayer(int x, int y) {
    ...
    if ( sqrt(sq(abs(this.x-x))+sq(abs(this.y-y))) >4) {
      findout =false;
    } else {
